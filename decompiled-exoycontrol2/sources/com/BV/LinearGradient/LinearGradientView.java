...
public class LinearGradientView extends View {
    ...
    private static float[] getHorizontalOrVerticalStartPoint(float f, int[] iArr) {
        float f2 = iArr[0] / 2.0f;
        float f3 = iArr[1] / 2.0f;
        return f == 0.0f ? new float[]{-f2, 0.0f} : f == 90.0f ? new float[]{0.0f, -f3} : f == 180.0f ? new float[]{f2, 0.0f} : new float[]{0.0f, f3};
    }
        ...
        if (iArr != null) {
            ...
            if (fArr3 == null || iArr.length == fArr3.length) {
                if (this.mUseAngle && this.mAngleCenter != null) {
                    ...
                    float f2 = f * iArr2[0];
                    float f3 = fArr4[1] * iArr2[1];
                    float[] fArr5 = {f2, f3};
                    fArr = new float[]{f2 + gradientStartPoint[0], f3 - gradientStartPoint[1]};
