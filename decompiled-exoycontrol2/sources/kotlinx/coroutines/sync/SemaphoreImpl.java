...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM2094constructorimpl;
        boolean z;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM2094constructorimpl = SegmentOrClosed.m2094constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM2094constructorimpl = SegmentOrClosed.m2094constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m2099isClosedimpl(objM2094constructorimpl)) {
                break;
            }
            Segment segmentM2097getSegmentimpl = SegmentOrClosed.m2097getSegmentimpl(objM2094constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= segmentM2097getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM2097getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (UByte$$ExternalSyntheticBackport0.m(tail$FU, this, segment, segmentM2097getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM2097getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM2097getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m2097getSegmentimpl(objM2094constructorimpl);
        int i = (int) (andIncrement % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (UByte$$ExternalSyntheticBackport0.m(semaphoreSegment3.acquirers, i, (Object) null, cont)) {
            cont.invokeOnCancellation(new CancelSemaphoreAcquisitionHandler(semaphoreSegment3, i));
            return true;
        }
        if (UByte$$ExternalSyntheticBackport0.m(semaphoreSegment3.acquirers, i, SemaphoreKt.PERMIT, SemaphoreKt.TAKEN)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (UByte$$ExternalSyntheticBackport0.m(head$FU, this, segment, segmentM2097getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM2097getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !UByte$$ExternalSyntheticBackport0.m(semaphoreSegment3.acquirers, i2, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
        }
